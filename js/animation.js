...
function initMenuReveal() {
    ...
    const allSections      = document.querySelectorAll("section, .menu-section, .category-block, .menu-category");
    ...
    const classSections = document.querySelectorAll(
        ".entrees, .plats, .desserts, .entree-section, .plats-section, .desserts-section, .menu-category"
    );
...
function _revealMenuBlock(section) {
    ...
    // 4. Images de plats : apparaissent en glissant depuis l'extérieur
    const imgRight = section.querySelectorAll(".category-img--right .dish-img");
    imgRight.forEach((img, i) => {
        img.style.opacity   = "0";
        img.style.transform = "translateX(50px) scale(0.8)";
        setTimeout(() => {
            animateEl(img,
                [
    ...
    const imgLeft = section.querySelectorAll(".category-img--left .dish-img");
    imgLeft.forEach((img, i) => {
        img.style.opacity   = "0";
        img.style.transform = "translateX(-50px) scale(0.8)";
        setTimeout(() => {
            animateEl(img,
...
function initFadeIn() {
    ...
    document.querySelectorAll("section").forEach((section, i) => {
        if (section.classList.contains("menu-category")) return;
        const heading = section.querySelector("h2, h3");
...
// ═══════════════════════════════════════════════════════════════════════════════
// 5b. HOVER IMAGES DE PLATS — Les images glissent vers l'extrémité au survol
// ═══════════════════════════════════════════════════════════════════════════════

function initDishImageHover() {
    document.querySelectorAll(".menu-category").forEach(section => {

        const imgsRight   = section.querySelectorAll(".category-img--right .dish-img");
        const imgsLeft    = section.querySelectorAll(".category-img--left .dish-img");
        const primaryR    = section.querySelector(".category-img--right .dish-img--primary");
        const secondaryR  = section.querySelector(".category-img--right .dish-img--secondary");
        const primaryL    = section.querySelector(".category-img--left .dish-img--primary");
        const secondaryL  = section.querySelector(".category-img--left .dish-img--secondary");

        section.addEventListener("mouseenter", () => {
            // Images droite : glissent vers la droite
            if (primaryR) {
                animateEl(primaryR,
                    [
                        { transform: "translateX(0) scale(1)",     filter: "brightness(0.97)", boxShadow: "0 4px 16px rgba(179,106,94,0.15)" },
                        { transform: "translateX(22px) scale(1.1)", filter: "brightness(1.06)", boxShadow: "0 10px 30px rgba(179,106,94,0.35)" }
                    ],
                    { duration: 350, fill: "forwards", easing: "cubic-bezier(0.34, 1.56, 0.64, 1)" }
                );
            }
            if (secondaryR) {
                animateEl(secondaryR,
                    [
                        { transform: "translateX(0) scale(1)",     opacity: "0.85" },
                        { transform: "translateX(13px) scale(1.06)", opacity: "1" }
                    ],
                    { duration: 350, delay: 60, fill: "forwards", easing: "cubic-bezier(0.34, 1.56, 0.64, 1)" }
                );
            }

            // Images gauche : glissent vers la gauche
            if (primaryL) {
                animateEl(primaryL,
                    [
                        { transform: "translateX(0) scale(1)",      filter: "brightness(0.97)", boxShadow: "0 4px 16px rgba(179,106,94,0.15)" },
        ...
        section.addEventListener("mouseleave", () => {
            ...
            [primaryR, secondaryR, primaryL, secondaryL].forEach(img => {
                if (!img) return;
                const isSecondary = img.classList.contains("dish-img--secondary");
                animateEl(img,
                    [
                        { transform: img.style.transform || "none" },
                        { transform: "translateX(0) scale(1)", filter: "brightness(0.97)", opacity: isSecondary ? "0.85" : "1" }
                    ],
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function _initInteractions() {
        ...
        initButtonHover();
        initSectionHover();
        initDishImageHover();   // ← Nouveau : hover images de plats
        initListItemHover();
        initCardHover();
        initRipple();
        return true;
